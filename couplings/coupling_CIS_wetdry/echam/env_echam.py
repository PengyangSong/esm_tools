"""
ECHAM env for awiesm-pism-vilma CIS when script_dir is couplings/coupling_CIS/echam.
"""
...
def prepare_environment(config):
    environment_dict = {
        ...
        "ISM_TO_ECHAM_update_glacial_mask": int(
            config["echam"].get("update_glacial_mask", True).__bool__()
        ),
        ...
        "HOSING_FILE_LANDICE_LOSS": config["fesom"].get(
            "fwf_path", config["general"]["experiment_couple_dir"]
        ),
        "HOSING_CORRECTION": int(
            config["echam"].get("hosing_correction", False).__bool__()
        ),
        "CELL_AREA_FESOM_FILE": config["fesom"].get(
            "fesom_cell_area_file", "fesom.mesh.diag.nc"
        ),
